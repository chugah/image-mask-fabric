  ...
  fabric.Maskedimage = fabric.util.createClass(fabric.Object, {
    ...
    _applyMask: function(imgElement) {  
      ...
      for (var i = 0, len = newCanvasElImageData.data.length; i < len; i += 4) {         
        ...
        if( this._pixelIsInOverlapRegion(px,py,overlapRegionRect)){
          newCanvasElImageData.data[i+3]=255;
        } else {
          newCanvasElImageData.data[i+3]=55;
        }
      }
      newCanvasElCtx.putImageData(newCanvasElImageData, 0, 0);
      return newCanvasEl;    
    },
    _drawCircle(ctxt,x,y){
      ctxt.save();
      ctxt.setTransform(1, 0, 0, 1, 0, 0);
      console.log("_drawCircle: x["+x+"] y["+y+"]");
      ctxt.beginPath();
      ctxt.arc(x, y, 11, 0, 2 * Math.PI, false);
      ctxt.fillStyle = 'red';
      ctxt.fill();
      ctxt.lineWidth = 2.5;
    ...
    _pixelIsInOverlapRegion: function( x,y, overlapRegionRect) {
      if( !overlapRegionRect) {
        //console.log("***overlapRegionRect undefined***");
        return false;
      }   
      TEST_COUNTER++;
      if( (TEST_COUNTER >= MIN_TEST_COUNT) && (TEST_COUNTER <= MAX_TEST_COUNT)){
        console.log("_pixelIsInOverlapRegion: x["+x+"]");
        console.log("_pixelIsInOverlapRegion: y["+y+"]");
        console.log("_pixelIsInOverlapRegion: overlapRegionRect.left["+overlapRegionRect.left+"]");
        console.log("_pixelIsInOverlapRegion: overlapRegionRect.top["+overlapRegionRect.top+"]");
        console.log("_pixelIsInOverlapRegion: overlapRegionRect.width["+overlapRegionRect.width+"]");
        console.log("_pixelIsInOverlapRegion: overlapRegionRect.height["+overlapRegionRect.height+"]");
        console.log("_pixelIsInOverlapRegion: overlapRegionRect.right[" + (overlapRegionRect.left + overlapRegionRect.width ) + "]");
        console.log("_pixelIsInOverlapRegion: overlapRegionRect.bottom["+ (overlapRegionRect.top + overlapRegionRect.height) +"]");
      }
      if(( x <= (overlapRegionRect.left+overlapRegionRect.width)) &&
        x >= overlapRegionRect.left &&
        y >= overlapRegionRect.top &&
        y <= ( overlapRegionRect.top + overlapRegionRect.height)) {
          if( (TEST_COUNTER >= MIN_TEST_COUNT) && (TEST_COUNTER <= MAX_TEST_COUNT)){
            console.log("_pixelIsInOverlapRegion: IN"); 
          }
          return true;
      } else {
        if( (TEST_COUNTER >= MIN_TEST_COUNT) && (TEST_COUNTER <= MAX_TEST_COUNT)){
          console.log("_pixelIsInOverlapRegion: OUT");            
        }
        return false;
      }
    },
